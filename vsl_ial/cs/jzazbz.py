...
M_LMS_INV = np.asfarray(
    [
        [+1.661373055774069e00, -9.145230923250668e-01, +2.313620767186147e-01],
        [-3.250758740427037e-01, +1.571847038366936e00, -2.182538318672940e-01],
        [-9.098281098284756e-02, -3.127282905230740e-01, +1.522766561305260e00],
    ]
)
...
M_IAZ_INV = np.asfarray(
    [
        [1.0, 1.386050432715393e-1, 5.804731615611869e-2],
        [1.0, -1.386050432715393e-1, -5.804731615611891e-2],
...
class JzAzBz(CS):
    ...
    def _from_XYZ(self, src: CS, color: FArray) -> FArray:
        def pqInv(X: FArray) -> FArray:
            ...
            return ((0.8359375 + 18.8515625 * XX) / (1 + 18.6875 * XX)) ** 134.034375

        LMSp = pqInv(np.tensordot(color, M_LMS.T, axes=1))
    ...
    def _to_XYZ(self, src: CS, color: FArray) -> FArray:
        ...
        def pqInv(X: FArray) -> FArray:
            ...
            return (
                1e4
                * ((0.8359375 - XX) / (18.6875 * XX - 18.8515625)) ** 6.277394636015326
            )

        LMS = pqInv(np.tensordot(color, M_IAZ_INV.T, axes=1))

        return np.tensordot(LMS, M_LMS_INV.T, axes=1)
